...
int isGz(unsigned char *memgz, long memgzlen) {
  ...
  if (memgz[memgzlen - 4] + memgz[memgzlen - 3] + memgz[memgzlen - 2] + memgz[memgzlen - 1] == 0)
    return (0); /* uncompressed file's len must be > 0 */
  if ((memgz[gzpos] != 0)
      && (memgz[gzpos++] != 8)) return (
                                           0); /* compression method should be 'store' (0) or 'deflate' (8) */
  ...
  *resultlen = 0;
  ...
  if (compmethod == 0) { /* if the file is stored, copy it over */
    ...
    while (extract_res == 0) {
      ...
      in_bytes = avail_in;
      out_bytes = avail_out;
      status = tinfl_decompress(tinflhandler, (const mz_uint8 *)next_in, &in_bytes, buffout,
                                (mz_uint8 *)next_out, &out_bytes, (infile_remaining ? TINFL_FLAG_HAS_MORE_INPUT : 0));
