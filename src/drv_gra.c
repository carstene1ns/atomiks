...
int gra_init(int width, int height, int flags, char *windowtitle, unsigned char *titleicon,
  ...
  window = SDL_CreateWindow(windowtitle, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width,
                            height, sdl_video_flags);
...
void gra_drawsprite_alpha(struct gra_sprite *sprite, int x, int y, int alpha) {
  ...
  if (alpha < 255) SDL_SetTextureAlphaMod(sprite->ptr, alpha);
  SDL_RenderCopy(renderer, sprite->ptr, NULL, &rect);
  if (alpha < 255) SDL_SetTextureAlphaMod(sprite->ptr, 255);
}
...
void loadSpriteSheet(struct gra_sprite **sprites, int width, int height, int itemcount,
  ...
  item = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0xFF000000L, 0x00FF0000L, 0x0000FF00L,
                              0x000000FFL);
